...
{
    typedef Ewoms::Constants<Scalar> Consts;
...
public:
    ...
    static Evaluation vaporPressure(const Evaluation& temperature)
    {
        ...
        return 100 * 1.334 * Ewoms::pow(10.0, A - (B / (T + C)));
    }
    ...
    {
        Evaluation T = Ewoms::min(temperature, criticalTemperature()); // regularization
        T = Ewoms::max(T, 0.0); // regularization
        ...
        const Evaluation& DH_vap = DH_v_boil * Ewoms::pow(((1.0 - Tr2)/(1.0 - Tr1)), n);
    ...
    static Evaluation gasViscosity(Evaluation temperature, const Evaluation& /*pressure*/, bool /*regularize*/=true)
    {
        ...
        temperature = Ewoms::max(temperature, 250.0);
        ...
        const Evaluation& eta_xi =
            Fp0*(0.807*Ewoms::pow(Tr,0.618)
                 - 0.357*Ewoms::exp(-0.449*Tr)
                 + 0.34*Ewoms::exp(-4.058*Tr)
                 + 0.018);
    ...
    static Evaluation liquidViscosity(Evaluation temperature, const Evaluation& /*pressure*/)
    {
        ...
        return Ewoms::exp(A + B/temperature)*1e-3; // [Pa s]
    }
...
protected:
    ...
    static Evaluation molarLiquidDensity_(Evaluation temperature)
    {
        ...
        const Evaluation& expo = 1.0 + Ewoms::pow(1.0 - temperature/criticalTemperature(), 2.0/7.0);
        const Evaluation& V = Consts::R*criticalTemperature()/criticalPressure()*Ewoms::pow(Z_RA, expo); // liquid molar volume [cm^3/mol]
