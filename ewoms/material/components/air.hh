...
public:
    ...
    static Evaluation gasViscosity(const Evaluation& temperature, const Evaluation& /*pressure*/)
    {
        ...
        Evaluation Omega_v =
            1.16145*Ewoms::pow(Tstar, -0.14874) +
            0.52487*Ewoms::exp(- 0.77320*Tstar) +
            2.16178*Ewoms::exp(- 2.43787*Tstar);
        return 40.7851e-7*Fc*Ewoms::sqrt(M*temperature)/(std::pow(Vc, 2./3)*Omega_v);
    }
    ...
    static Evaluation simpleGasViscosity(const Evaluation& temperature, const Evaluation& /*pressure*/)
    {
        if(temperature < 273.15 || temperature > 660.) {
            throw NumericalIssue("Air: Temperature "+std::to_string(Ewoms::scalarValue(temperature))+"K out of range");
        }
        return 1.496e-6*Ewoms::pow(temperature, 1.5)/(temperature + 120);
    }
