...
public:
                                     ...
                                     const Evaluation& pressure)
    {
        ...
        const Evaluation& S_lSAT =
            f[0]
            + f[1]*theta
            + f[2]*Ewoms::pow(theta, 2)
            + f[3]*Ewoms::pow(theta, 3);
        ...
        const Evaluation& h_NaCl =
            (3.6710e4*temperature
             ...
             + (2.8000e-5/4)*Ewoms::pow(temperature, 4.0))/58.44e3
            - 2.045698e+02; // [kJ/kg]
        ...
        for (int i = 0; i<=3; ++i) {
            for (int j = 0; j <= 2; ++j) {
                d_h += a[i][j] * Ewoms::pow(theta, i) * Ewoms::pow(m, j);
            }
        }
    ...
    static Evaluation liquidHeatCapacity(const Evaluation& temperature,
                                         const Evaluation& pressure)
    {
        Scalar eps = Ewoms::scalarValue(temperature)*1e-8;
    ...
    static Evaluation liquidPressure(const Evaluation& temperature, const Evaluation& density)
    {
        ...
        Scalar eps = Ewoms::scalarValue(pressure)*1e-7;

        Evaluation deltaP = pressure*2;
        for (int i = 0;
             i < 5
                 && std::abs(Ewoms::scalarValue(pressure)*1e-9) < std::abs(Ewoms::scalarValue(deltaP));
             ++i)
        {
    ...
    static Evaluation liquidViscosity(const Evaluation& temperature, const Evaluation& /*pressure*/)
    {
        ...
        Evaluation A = (0.42*std::pow((std::pow(salinity, 0.8)-0.17), 2) + 0.045)*Ewoms::pow(T_C, 0.8);
        Evaluation mu_brine = 0.1 + 0.333*salinity + (1.65+91.9*salinity*salinity*salinity)*Ewoms::exp(-A);
