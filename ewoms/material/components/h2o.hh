...
public:
                                           ...
                                           const Evaluation& pressure)
    {
        ...
        // regularization
        Scalar pv = vaporPressure<Scalar>(Ewoms::scalarValue(temperature));
        if (pressure < pv) {
            // the pressure is too low, in this case we use the slope
    ...
    static Evaluation gasInternalEnergy(const Evaluation& temperature, const Evaluation& pressure)
    {
        ...
        Scalar pv = vaporPressure(Ewoms::scalarValue(temperature));
        if (pressure > pv) {
            // the pressure is too high, in this case we use the slope
            // of the internal energy at the vapor pressure to
            // regularize
    ...
    static Evaluation gasDensity(const Evaluation& temperature, const Evaluation& pressure)
    {
        ...
        if (pressure > pv) {
            ...
            // to the pressure at the vapor pressure.
            Scalar eps = Ewoms::scalarValue(pv)*1e-8;
    ...
    static Evaluation gasPressure(const Evaluation& temperature, Scalar density)
    {
        ...
        for (int i = 0; i < 5 && std::abs(Ewoms::scalarValue(pressure)*1e-9) < std::abs(Ewoms::scalarValue(deltaP)); ++i) {
...
private:
    ...
    {
        return
            - Ewoms::pow(Region1::tau(temperature), 2.0) *
    ...
    {
        return
            - Ewoms::pow(Region2::tau(temperature), 2 ) *
