...
public:
    ...
    static Evaluation saturationPressure(const Evaluation& temperature)
    {
        ...
        Evaluation tmp = 2*C/(Ewoms::sqrt(B*B - 4*A*C) - B);
        tmp *= tmp;
        tmp *= tmp;
    ...
    static Evaluation vaporTemperature(const Evaluation& pressure)
    {
        ...
        const Evaluation& D = ( 2.*G)/(-F -Ewoms::sqrt(pow(F,2.) - 4.*E*G));

        const Evaluation& temperature = (n[9] + D - Ewoms::sqrt(pow(n[9]+D , 2.) - 4.* (n[8] + n[9]*D)) ) * 0.5;
