...
public:
    ...
    static Evaluation viscosity(const Evaluation& temperature, const Evaluation& rho)
    {
        ...
        muBar *= rhoBar;
        muBar = Ewoms::exp(muBar);

        // muBar *= muBar_0
        muBar  *= 100*Ewoms::sqrt(TBar);
        const Scalar H[4] = {
    ...
    */
    ...
    static Evaluation thermalConductivityIAPWS(const Evaluation& T, const Evaluation& rho)
    {
        ...
        Evaluation Troot = Ewoms::sqrt(Tbar);
        ...
        lam +=
            thcond_b0 + thcond_b1
            * rhobar + thcond_b2
            * Ewoms::exp(thcond_B1 * ((rhobar + thcond_B2)*(rhobar + thcond_B2)));

        Evaluation DTbar = Ewoms::abs(Tbar - 1) + thcond_c4;
        Evaluation DTbarpow = Ewoms::pow(DTbar, 3./5);
        ...
        Evaluation rhobar18 = Ewoms::pow(rhobar, 1.8);
        Evaluation rhobarQ = Ewoms::pow(rhobar, Q);

        lam +=
            (thcond_d1 / Ewoms::pow(Tbar,10.0) + thcond_d2) * rhobar18 *
            Ewoms::exp(thcond_c1 * (1 - rhobar * rhobar18))
            + thcond_d3 * S * rhobarQ *
            Ewoms::exp((Q/(1+Q))*(1 - rhobar*rhobarQ))
            + thcond_d4 *
            Ewoms::exp(thcond_c2 * Ewoms::pow(Troot,3.0) + thcond_c3 / Ewoms::pow(rhobar,5.0));
