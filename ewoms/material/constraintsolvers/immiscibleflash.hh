...
public:
                      ...
                      Scalar tolerance = -1)
    {
        ...
        typedef Ewoms::DenseAd::Evaluation<InputEval, numEq> FlashEval;
        typedef Dune::FieldVector<FlashEval, numEq> FlashDefectVector;
        typedef Ewoms::ImmiscibleFluidState<FlashEval, FluidSystem> FlashFluidState;
        ...
        for (unsigned nIdx = 0; nIdx < nMax; ++nIdx) {
            ...
            try { J.solve(deltaX, b); }
                ...
                throw Ewoms::NumericalIssue(e.what());
            }
    ...
    throw Ewoms::NumericalIssue(oss.str());
    }
...
protected:
                          ...
                          const EvalVector& deltaX)
    {
        ...
        typedef typename FlashFluidState::Scalar FlashEval;
        typedef Ewoms::MathToolbox<FlashEval> FlashEvalToolbox;
        ...
        for (unsigned i = 0; i < numEq; ++i)
            assert(std::isfinite(Ewoms::scalarValue(deltaX[i])));
#endif
