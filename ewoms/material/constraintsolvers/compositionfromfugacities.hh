...
/*!
 * \file
 * \copydoc Ewoms::CompositionFromFugacities
 */
...
protected:
                             ...
                             const ComponentVector& targetFug)
    {
        ...
        for (unsigned i = 0; i < numComponents; ++ i) {
            ...
            absError = std::max(absError, std::abs(Ewoms::scalarValue(defect[i])));
        }
                          ...
                          const Dune::FieldVector<Evaluation, numComponents>& targetFug)
    {
        ...
        for (unsigned i = 0; i < numComponents; ++i) {
            ...
            relError = std::max(relError, std::abs(Ewoms::scalarValue(x[i])));

            sumx += Ewoms::abs(fluidState.moleFraction(phaseIdx, i));
            sumDelta += Ewoms::abs(x[i]);
        }
