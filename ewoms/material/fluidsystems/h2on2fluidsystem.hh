...
/*!
 * \file
 * \copydoc Ewoms::H2ON2FluidSystem
 */
...
public:
                           ...
                           unsigned phaseIdx)
    {
        ...
        for (unsigned compIdx = 0; compIdx < numComponents; ++compIdx)
            sumMoleFrac += Ewoms::decay<LhsEval>(fluidState.moleFraction(phaseIdx, compIdx));
        ...
        if (phaseIdx == liquidPhaseIdx) {
            ...
            const auto& xlH2O = Ewoms::decay<LhsEval>(fluidState.moleFraction(liquidPhaseIdx, H2OIdx));
            const auto& xlN2 = Ewoms::decay<LhsEval>(fluidState.moleFraction(liquidPhaseIdx, N2Idx));
        ...
        const auto& xgN2 = Ewoms::decay<LhsEval>(fluidState.moleFraction(gasPhaseIdx, N2Idx));
        const auto& rho_gH2O = H2O::gasDensity(T, p*xgH2O);
        const auto& rho_gN2 = N2::gasDensity(T, p*xgN2);
        return (rho_gH2O + rho_gN2)/Ewoms::max(1e-5, sumMoleFrac);
    }
                             ...
                             unsigned phaseIdx)
    {
        ...
        LhsEval sumx = 0.0;
        for (unsigned compIdx = 0; compIdx < numComponents; ++compIdx)
            sumx += Ewoms::decay<LhsEval>(fluidState.moleFraction(phaseIdx, compIdx));
        sumx = Ewoms::max(1e-10, sumx);

        for (unsigned i = 0; i < numComponents; ++i) {
            LhsEval divisor = 0;
            for (unsigned j = 0; j < numComponents; ++j) {
                LhsEval phiIJ = 1 + Ewoms::sqrt(mu[i]/mu[j]) * std::pow(molarMass(j)/molarMass(i), 1/4.0);
                phiIJ *= phiIJ;
                phiIJ /= std::sqrt(8*(1 + molarMass(i)/molarMass(j)));
                divisor +=
                    Ewoms::decay<LhsEval>(fluidState.moleFraction(phaseIdx, j))
                    /sumx*phiIJ;
            }
            muResult +=
                Ewoms::decay<LhsEval>(fluidState.moleFraction(phaseIdx, i))
                /sumx*mu[i]/divisor;
        }
        return muResult;
    }
                            ...
                            unsigned phaseIdx)
    {
        ...
        const auto& XgH2O = Ewoms::decay<LhsEval>(fluidState.massFraction(gasPhaseIdx, H2OIdx));
        const auto& XgN2 = Ewoms::decay<LhsEval>(fluidState.massFraction(gasPhaseIdx, N2Idx));
                                       ...
                                       unsigned phaseIdx)
    {
        ...
        const auto& xH2O = Ewoms::decay<LhsEval>(fluidState.moleFraction(phaseIdx, H2OIdx));
        const auto& xN2 = Ewoms::decay<LhsEval>(fluidState.moleFraction(phaseIdx, N2Idx));
                                ...
                                unsigned phaseIdx)
    {
        const auto& T = Ewoms::decay<LhsEval>(fluidState.temperature(phaseIdx));
        const auto& p = Ewoms::decay<LhsEval>(fluidState.pressure(phaseIdx));
        const auto& xAlphaH2O = Ewoms::decay<LhsEval>(fluidState.moleFraction(phaseIdx, H2OIdx));
        const auto& xAlphaN2 = Ewoms::decay<LhsEval>(fluidState.moleFraction(phaseIdx, N2Idx));
        const auto& XAlphaH2O = Ewoms::decay<LhsEval>(fluidState.massFraction(phaseIdx, H2OIdx));
        const auto& XAlphaN2 = Ewoms::decay<LhsEval>(fluidState.massFraction(phaseIdx, N2Idx));
