...
/*!
 * \file
 * \copydoc Ewoms::H2OAirFluidSystem
 */
...
public:
                           ...
                           unsigned phaseIdx)
    {
        ...
        if (isCompressible(phaseIdx))
            p = Ewoms::decay<LhsEval>(fluidState.pressure(phaseIdx));
        else {
        ...
        if (phaseIdx == liquidPhaseIdx)
        {
            ...
            const auto& xlAir = Ewoms::decay<LhsEval>(fluidState.moleFraction(liquidPhaseIdx, AirIdx));
        ...
        else if (phaseIdx == gasPhaseIdx)
        {
            LhsEval partialPressureH2O =
                Ewoms::decay<LhsEval>(fluidState.moleFraction(gasPhaseIdx, H2OIdx))
                *Ewoms::decay<LhsEval>(fluidState.pressure(gasPhaseIdx));

            LhsEval partialPressureAir =
                Ewoms::decay<LhsEval>(fluidState.moleFraction(gasPhaseIdx, AirIdx))
                *Ewoms::decay<LhsEval>(fluidState.pressure(gasPhaseIdx));
        ...
        else if (phaseIdx == gasPhaseIdx)
        {
            ...
            for (unsigned i = 0; i < numComponents; ++i) {
                ...
                for (unsigned j = 0; j < numComponents; ++j) {
                    LhsEval phiIJ =
                        1 +
                        Ewoms::sqrt(mu[i]/mu[j]) * // 1 + (mu[i]/mu[j]^1/2
                        std::pow(molarMass(j)/molarMass(i), 1./4.0);   // (M[i]/M[j])^1/4

                    phiIJ *= phiIJ;
                    phiIJ /= std::sqrt(8*(1 + molarMass(i)/molarMass(j)));
                    divisor += Ewoms::decay<LhsEval>(fluidState.moleFraction(phaseIdx, j))*phiIJ;
                }
                const auto& xAlphaI = Ewoms::decay<LhsEval>(fluidState.moleFraction(phaseIdx, i));
                            ...
                            unsigned phaseIdx)
    {
        ...
        else if (phaseIdx == gasPhaseIdx)
        {
            ...
            result +=
                H2O::gasEnthalpy(T, p) *
                Ewoms::decay<LhsEval>(fluidState.massFraction(gasPhaseIdx, H2OIdx));

            result +=
                Air::gasEnthalpy(T, p) *
                Ewoms::decay<LhsEval>(fluidState.massFraction(gasPhaseIdx, AirIdx));
            return result;
        }
        throw std::logic_error("Invalid phase index "+std::to_string(phaseIdx));
    }
                                       ...
                                       unsigned phaseIdx)
    {
        ...
        const LhsEval& temperature =
            Ewoms::decay<LhsEval>(fluidState.temperature(phaseIdx));
        ...
        if (phaseIdx == liquidPhaseIdx)
            ...
            const LhsEval& xAir =
                Ewoms::decay<LhsEval>(fluidState.moleFraction(phaseIdx, AirIdx));
            const LhsEval& xH2O =
                Ewoms::decay<LhsEval>(fluidState.moleFraction(phaseIdx, H2OIdx));
