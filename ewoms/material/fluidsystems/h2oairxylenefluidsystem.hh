...
/*!
 * \file
 * \copydoc Ewoms::H2OAirXyleneFluidSystem
 */
...
#include <ewoms/material/binarycoefficients/h2o_xylene.hh>
#include <ewoms/material/binarycoefficients/air_xylene.hh>
...
public:
    ...
    typedef Ewoms::Xylene<Scalar> NAPL;
                           ...
                           unsigned phaseIdx)
    {
        if (phaseIdx == waterPhaseIdx) {
            ...
            const auto& xwH2O = Ewoms::decay<LhsEval>(fluidState.moleFraction(waterPhaseIdx, H2OIdx));
            const auto& xwAir = Ewoms::decay<LhsEval>(fluidState.moleFraction(waterPhaseIdx, airIdx));
            const auto& xwNapl = Ewoms::decay<LhsEval>(fluidState.moleFraction(waterPhaseIdx, NAPLIdx));
        ...
        const LhsEval& pH2O = Ewoms::decay<LhsEval>(fluidState.moleFraction(gasPhaseIdx, H2OIdx))*p;
        const LhsEval& pAir = Ewoms::decay<LhsEval>(fluidState.moleFraction(gasPhaseIdx, airIdx))*p;
        const LhsEval& pNAPL = Ewoms::decay<LhsEval>(fluidState.moleFraction(gasPhaseIdx, NAPLIdx))*p;
        return
                             ...
                             unsigned phaseIdx)
    {
        ...
        const auto& xgAir = Ewoms::decay<LhsEval>(fluidState.moleFraction(gasPhaseIdx, airIdx));
        const auto& xgH2O = Ewoms::decay<LhsEval>(fluidState.moleFraction(gasPhaseIdx, H2OIdx));
        const auto& xgNapl = Ewoms::decay<LhsEval>(fluidState.moleFraction(gasPhaseIdx, NAPLIdx));
                                        ...
                                        unsigned compIdx)
    {
        if (phaseIdx==gasPhaseIdx) {
            ...
            const LhsEval& diffAC = Ewoms::BinaryCoeff::Air_Xylene::gasDiffCoeff(T, p);
            const LhsEval& diffWC = Ewoms::BinaryCoeff::H2O_Xylene::gasDiffCoeff(T, p);
                                       ...
                                       unsigned compIdx)
    {
        ...
        if (phaseIdx == waterPhaseIdx) {
            if (compIdx == H2OIdx)
                ...
                return Ewoms::BinaryCoeff::H2O_Xylene::henry(T)/p;
        }
                            ...
                            unsigned phaseIdx)
    {
        ...
        if (phaseIdx == waterPhaseIdx) {
            ...
            LhsEval result = 0;
            result += hgw * Ewoms::decay<LhsEval>(fluidState.massFraction(gasPhaseIdx, H2OIdx));
            result += hga * Ewoms::decay<LhsEval>(fluidState.massFraction(gasPhaseIdx, airIdx));
            result += hgc * Ewoms::decay<LhsEval>(fluidState.massFraction(gasPhaseIdx, NAPLIdx));
