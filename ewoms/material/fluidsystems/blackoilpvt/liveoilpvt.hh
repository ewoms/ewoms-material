...
/*!
 * \file
 * \copydoc Ewoms::LiveOilPvt
 */
#ifndef EWOMS_LIVE_OIL_PVT_HH
#define EWOMS_LIVE_OIL_PVT_HH
...
public:
                                             ...
                                             Evaluation maxOilSaturation) const
    {
        ...
        // keyword)
        maxOilSaturation = Ewoms::min(maxOilSaturation, Scalar(1.0));
        if (vapPar2_ > 0.0 && maxOilSaturation > 0.01 && oilSaturation < maxOilSaturation) {
            static const Scalar eps = 0.001;
            const Evaluation& So = Ewoms::max(oilSaturation, eps);
            tmp *= Ewoms::max(1e-3, Ewoms::pow(So/maxOilSaturation, vapPar2_));
        }
                                  ...
                                  const Evaluation& Rs) const
    {
        ...
        for (int i = 0; i < 20; ++i) {
            ...
            if (std::abs(Ewoms::scalarValue(fPrime)) < 1.0e-30) {
                return pSat;
            }
            ...
            if (std::abs(Ewoms::scalarValue(delta)) < std::abs(Ewoms::scalarValue(pSat))*eps)
                return pSat;
        }
...
private:
    void updateSaturationPressure_(unsigned regionIdx)
    {
        ...
        auto xCoordComparator = [](const Pair& a, const Pair& b) { return a.first == b.first; };
        auto last = std::unique(pSatSamplePoints.begin(), pSatSamplePoints.end(), xCoordComparator);
