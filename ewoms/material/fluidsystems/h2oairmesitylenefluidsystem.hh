...
/*!
 * \file
 * \copydoc Ewoms::H2OAirMesityleneFluidSystem
 */
...
#include <ewoms/material/binarycoefficients/h2o_mesitylene.hh>
#include <ewoms/material/binarycoefficients/air_mesitylene.hh>
    ...
    : public BaseFluidSystem<Scalar, H2OAirMesityleneFluidSystem<Scalar> >
{
    ...
    typedef Ewoms::TabulatedComponent<Scalar, IapwsH2O, /*alongVaporPressure=*/false> TabulatedH2O;
...
public:
    ...
    typedef Ewoms::Mesitylene<Scalar> NAPL;
                           ...
                           unsigned phaseIdx)
    {
        ...
        if (phaseIdx == waterPhaseIdx) {
            ...
            const LhsEval& p =
                H2O::liquidIsCompressible()
                ? Ewoms::decay<LhsEval>(fluidState.pressure(phaseIdx))
                : 1e30;
            ...
            return
                clH2O*(H2O::molarMass()*Ewoms::decay<LhsEval>(fluidState.moleFraction(waterPhaseIdx, H2OIdx)) +
                       Air::molarMass()*Ewoms::decay<LhsEval>(fluidState.moleFraction(waterPhaseIdx, airIdx)) +
                       NAPL::molarMass()*Ewoms::decay<LhsEval>(fluidState.moleFraction(waterPhaseIdx, NAPLIdx)));
        }
        ...
        const LhsEval& pg = Ewoms::decay<LhsEval>(fluidState.pressure(gasPhaseIdx));
        const LhsEval& pH2O = Ewoms::decay<LhsEval>(fluidState.moleFraction(gasPhaseIdx, H2OIdx))*pg;
        const LhsEval& pAir = Ewoms::decay<LhsEval>(fluidState.moleFraction(gasPhaseIdx, airIdx))*pg;
        const LhsEval& pNAPL = Ewoms::decay<LhsEval>(fluidState.moleFraction(gasPhaseIdx, NAPLIdx))*pg;
        return
                             ...
                             unsigned phaseIdx)
    {
        ...
        const LhsEval& xgAir = Ewoms::decay<LhsEval>(fluidState.moleFraction(gasPhaseIdx, airIdx));
        const LhsEval& xgH2O = Ewoms::decay<LhsEval>(fluidState.moleFraction(gasPhaseIdx, H2OIdx));
        const LhsEval& xgNapl = Ewoms::decay<LhsEval>(fluidState.moleFraction(gasPhaseIdx, NAPLIdx));
                                        ...
                                        unsigned /*compIdx*/)
    {
        ...
        if (phaseIdx==gasPhaseIdx) {
            const LhsEval& diffAC = Ewoms::BinaryCoeff::Air_Mesitylene::gasDiffCoeff(T, p);
            const LhsEval& diffWC = Ewoms::BinaryCoeff::H2O_Mesitylene::gasDiffCoeff(T, p);
            const LhsEval& diffAW = Ewoms::BinaryCoeff::H2O_Air::gasDiffCoeff(T, p);

            const LhsEval& xga = Ewoms::decay<LhsEval>(fluidState.moleFraction(gasPhaseIdx, airIdx));
            const LhsEval& xgw = Ewoms::decay<LhsEval>(fluidState.moleFraction(gasPhaseIdx, H2OIdx));
            const LhsEval& xgc = Ewoms::decay<LhsEval>(fluidState.moleFraction(gasPhaseIdx, NAPLIdx));
            ...
            const LhsEval& xwa = Ewoms::decay<LhsEval>(fluidState.moleFraction(waterPhaseIdx, airIdx));
            const LhsEval& xww = Ewoms::decay<LhsEval>(fluidState.moleFraction(waterPhaseIdx, H2OIdx));
            const LhsEval& xwc = Ewoms::decay<LhsEval>(fluidState.moleFraction(waterPhaseIdx, NAPLIdx));
                                       ...
                                       unsigned compIdx)
    {
        ...
        if (phaseIdx == waterPhaseIdx) {
            if (compIdx == H2OIdx)
                ...
                return Ewoms::BinaryCoeff::H2O_N2::henry(T)/p;
            else if (compIdx == NAPLIdx)
                return Ewoms::BinaryCoeff::H2O_Mesitylene::henry(T)/p;
            assert(false);
        }
                            ...
                            unsigned phaseIdx)
    {
        ...
        if (phaseIdx == waterPhaseIdx) {
            ...
            LhsEval result = 0;
            result += H2O::gasEnthalpy(T, p) * Ewoms::decay<LhsEval>(fluidState.massFraction(gasPhaseIdx, H2OIdx));
            result += NAPL::gasEnthalpy(T, p) * Ewoms::decay<LhsEval>(fluidState.massFraction(gasPhaseIdx, airIdx));
            result += Air::gasEnthalpy(T, p) * Ewoms::decay<LhsEval>(fluidState.massFraction(gasPhaseIdx, NAPLIdx));
