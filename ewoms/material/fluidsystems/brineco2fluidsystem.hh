 ...
 * \file
 *
 * \copydoc Ewoms::BrineCO2FluidSystem
 */
...
#include <ewoms/material/binarycoefficients/h2o_co2.hh>
...
{
    typedef Ewoms::H2O<Scalar> H2O_IAPWS;
    typedef Ewoms::Brine<Scalar, H2O_IAPWS> Brine_IAPWS;
    typedef Ewoms::TabulatedComponent<Scalar, H2O_IAPWS> H2O_Tabulated;
    typedef Ewoms::TabulatedComponent<Scalar, Brine_IAPWS> Brine_Tabulated;
...
public:
    ...
    typedef Ewoms::BinaryCoeff::Brine_CO2<Scalar, CO2Tables> BinaryCoeffBrineCO2;
     ...
     */
    static bool isIdealMixture(unsigned phaseIdx EWOMS_OPTIM_UNUSED)
    {
        assert(0 <= phaseIdx && phaseIdx < numPhases);
     ...
     */
    static bool isCompressible(unsigned phaseIdx EWOMS_OPTIM_UNUSED)
    {
        assert(0 <= phaseIdx && phaseIdx < numPhases);
                           ...
                           unsigned phaseIdx)
    {
        ...
        const LhsEval& temperature = Ewoms::decay<LhsEval>(fluidState.temperature(phaseIdx));
        const LhsEval& pressure = Ewoms::decay<LhsEval>(fluidState.pressure(phaseIdx));
        ...
        if (phaseIdx == liquidPhaseIdx) {
            ...
            LhsEval xlBrine = Ewoms::min(1.0, Ewoms::max(0.0, Ewoms::decay<LhsEval>(fluidState.moleFraction(liquidPhaseIdx, BrineIdx))));
            LhsEval xlCO2 = Ewoms::min(1.0, Ewoms::max(0.0,  Ewoms::decay<LhsEval>(fluidState.moleFraction(liquidPhaseIdx, CO2Idx))));
        ...
        // (the relations don't seem to take non-normalized
        // compositions too well...)
        LhsEval xgBrine = Ewoms::min(1.0, Ewoms::max(0.0, Ewoms::decay<LhsEval>(fluidState.moleFraction(gasPhaseIdx, BrineIdx))));
        LhsEval xgCO2 = Ewoms::min(1.0, Ewoms::max(0.0,  Ewoms::decay<LhsEval>(fluidState.moleFraction(gasPhaseIdx, CO2Idx))));
                                       ...
                                       unsigned compIdx)
    {
        ...
        xlCO2 = Ewoms::max(0.0, Ewoms::min(1.0, xlCO2));
        xgH2O = Ewoms::max(0.0, Ewoms::min(1.0, xgH2O));
                            ...
                            unsigned phaseIdx)
    {
        ...
        if (phaseIdx == liquidPhaseIdx) {
            const LhsEval& XlCO2 = Ewoms::decay<LhsEval>(fluidState.massFraction(phaseIdx, CO2Idx));
            ...
            const LhsEval& XCO2 = Ewoms::decay<LhsEval>(fluidState.massFraction(gasPhaseIdx, CO2Idx));
            const LhsEval& XBrine = Ewoms::decay<LhsEval>(fluidState.massFraction(gasPhaseIdx, BrineIdx));
...
private:
                                           ...
                                           const LhsEval& X_CO2_w)
    {
        ...
        // Regularization
        Scalar scalarTheta = Ewoms::scalarValue(theta);
        ...
        for (i = 0; i<=3; i++) {
            for (j=0; j<=2; j++) {
                d_h = d_h + a[i][j] * Ewoms::pow(theta, static_cast<Scalar>(i)) * std::pow(m, j);
            }
        }
