...
/*!
 * \file
 * \copydoc Ewoms::PengRobinsonParamsMixture
 */
...
public:
    ...
    void updatePure(Scalar temperature, Scalar pressure)
    {
        ...
        for (unsigned i = 0; i < numComponents; ++i) {
            ...
            Scalar tmp = 1 + f_omega*(1 - Ewoms::sqrt(Tr));
            tmp = tmp*tmp;

            Scalar newA = 0.4572355*RTc*RTc/pc * tmp;
            Scalar newB = 0.0777961 * RTc / pc;
            assert(std::isfinite(Ewoms::scalarValue(newA)));
            assert(std::isfinite(Ewoms::scalarValue(newB)));
    ...
    void updateMix(const FluidState& fs)
    {
        ...
        for (unsigned compIIdx = 0; compIIdx < numComponents; ++compIIdx) {
            ...
            Scalar xi = Ewoms::max(0.0, Ewoms::min(1.0, moleFracI));
            ...
            for (unsigned compJIdx = 0; compJIdx < numComponents; ++compJIdx) {
                ...
                Scalar xj = Ewoms::max(0.0, Ewoms::min(1.0, moleFracJ));
            ...
            // mixing rule from Reid, page 82
            newB += Ewoms::max(0.0, xi) * this->pureParams_[compIIdx].b();
            assert(std::isfinite(Ewoms::scalarValue(newB)));
        }
        ...
        for (unsigned compIIdx = 0; compIIdx < numComponents; ++ compIIdx) {
            for (unsigned compJIdx = 0; compJIdx < numComponents; ++ compJIdx) {
                ...
                aCache_[compIIdx][compJIdx] =
                    Ewoms::sqrt(this->pureParams_[compIIdx].a()
