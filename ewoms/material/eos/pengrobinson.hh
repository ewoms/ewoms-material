...
#include <ewoms/common/polynomialutils.hh>
...
/*
    ...
    static Evaluation computeVaporPressure(const Params& params, const Evaluation& T)
    {
        ...
        for (unsigned i = 0; i < 5; ++i) {
            ...
            int numSol EWOMS_OPTIM_UNUSED = molarVolumes(Vm, params, T, pVap);
            assert(numSol == 3);
            ...
            if (std::abs(Ewoms::scalarValue(delta/pVap)) < 1e-10)
                break;
        }
                       ...
                       bool isGasPhase)
    {
        ...
        if (!std::isfinite(Ewoms::scalarValue(a))
            || std::abs(Ewoms::scalarValue(a)) < 1e-30)
            return std::numeric_limits<Scalar>::quiet_NaN();
        if (!std::isfinite(Ewoms::scalarValue(b)) || b <= 0)
            return std::numeric_limits<Scalar>::quiet_NaN();
        ...
        assert(std::isfinite(Ewoms::scalarValue(Vm)));
        assert(Vm > 0);
        return Vm;
    }
...
protected:
                                     ...
                                     bool isGasPhase)
    {
        ...
        if (isGasPhase)
            Vm = Ewoms::max(Vm, Vcrit);
        else
            Vm = Ewoms::min(Vm, Vcrit);
    }
                                   ...
                                   const Evaluation& b)
    {
        ...
        for (unsigned i = 0; i < iMax; ++i) {
            ...
            bool hasExtrema EWOMS_OPTIM_UNUSED = findExtrema_(minVm, maxVm, minP, maxP, a, b, T + eps);
            assert(hasExtrema);
            assert(std::isfinite(Ewoms::scalarValue(maxVm)));
            ...
            if (std::abs(Ewoms::scalarValue(fPrime)) < 1e-40) {
                Tcrit = T;
                pcrit = (minP + maxP)/2;
                Vcrit = (maxVm + minVm)/2;
                return;
            ...
            assert(std::isfinite(Ewoms::scalarValue(delta)));
            if (delta > 0)
                delta = -10;
                             ...
                             const Evaluation& T)
    {
        ...
        assert(std::isfinite(Ewoms::scalarValue(a1)));
        assert(std::isfinite(Ewoms::scalarValue(a2)));
        assert(std::isfinite(Ewoms::scalarValue(a3)));
        assert(std::isfinite(Ewoms::scalarValue(a4)));
        assert(std::isfinite(Ewoms::scalarValue(a5)));
        ...
        for (unsigned i = 0; std::abs(Ewoms::scalarValue(delta)) > 1e-12; ++i) {
            const Evaluation& f = a5 + V*(a4 + V*(a3 + V*(a2 + V*a1)));
            const Evaluation& fPrime = a4 + V*(2*a3 + V*(3*a2 + V*4*a1));

            if (std::abs(Ewoms::scalarValue(fPrime)) < 1e-20) {
        ...
        assert(std::isfinite(Ewoms::scalarValue(V)));
        ...
        Evaluation allV[4];
        allV[0] = V;
        int numSol = 1 + Ewoms::invertCubicPolynomial<Evaluation>(allV + 1, b1, b2, b3, b4);
    ...
    static Evaluation ambroseWalton_(const Params& /*params*/, const Evaluation& T)
    {
        ...
        const Evaluation& f0 = (tau*(-5.97616 + Ewoms::sqrt(tau)*(1.29874 - tau*0.60394)) - 1.06841*Ewoms::pow(tau, 5))/Tr;
        const Evaluation& f1 = (tau*(-5.03365 + Ewoms::sqrt(tau)*(1.11505 - tau*5.41217)) - 7.46628*Ewoms::pow(tau, 5))/Tr;
        const Evaluation& f2 = (tau*(-0.64771 + Ewoms::sqrt(tau)*(2.41539 - tau*4.26979)) + 3.25259*Ewoms::pow(tau, 5))/Tr;
