...
/*!
 * \file
 * \copydoc Ewoms::NullMaterial
 */
#ifndef EWOMS_NULL_MATERIAL_HH
#define EWOMS_NULL_MATERIAL_HH

#include "nullmaterialparams.hh"
...
public:
                                       ...
                                       const FluidState& fluidState)
    {
        ...
        for (unsigned phaseIdx = 0; phaseIdx < numPhases; ++phaseIdx) {
            const Evaluation& S =
                Ewoms::decay<Evaluation>(fluidState.saturation(phaseIdx));
            values[phaseIdx] = Ewoms::max(Ewoms::min(S, 1.0), 0.0);
        }
    }
    ...
    {
        const Evaluation& Sw =
            Ewoms::decay<Evaluation>(fluidState.saturation(Traits::wettingPhaseIdx));

        return Ewoms::max(0.0, Ewoms::min(1.0, Sw));
    }
    ...
    {
        const Evaluation& Sn =
            Ewoms::decay<Evaluation>(fluidState.saturation(Traits::nonWettingPhaseIdx));

        return Ewoms::max(0.0, Ewoms::min(1.0, Sn));
    }
    ...
    {
        return Ewoms::max(0.0, Ewoms::min(1.0, 1.0 - Ewoms::decay<Evaluation>(Sw)));
    }
    ...
    {
        const Evaluation& Sg =
            Ewoms::decay<Evaluation>(fluidState.saturation(Traits::gasPhaseIdx));

        return Ewoms::max(0.0, Ewoms::min(1.0, Sg));
    }
