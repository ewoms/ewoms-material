...
/*!
 * \file
 * \copydoc Ewoms::EclMaterialLawManager
 */
#if ! HAVE_ECL_INPUT
#error "Eclipse input support in ewoms-eclio is required to use the ECL material manager!"
#endif

#ifndef EWOMS_ECL_MATERIAL_LAW_MANAGER_HH
#define EWOMS_ECL_MATERIAL_LAW_MANAGER_HH
...
#include <ewoms/material/fluidmatrixinteractions/eclepsconfig.hh>
#include <ewoms/material/fluidmatrixinteractions/eclhysteresisconfig.hh>
...
public:
                         ...
                         Scalar Sw)
    {
        ...
        if (pcow < 0.0)
            ...
            if (std::abs(pcowAtSw) > pcowAtSwThreshold) {
                ...
                elemEclEpsScalingPoints.init(elemScaledEpsInfo, *oilWaterEclEpsConfig_, Ewoms::EclOilWaterSystem);
            }
        }
    ...
    const MaterialLawParams& connectionMaterialLawParams(unsigned satRegionIdx, unsigned elemIdx) const
    {
        ...
        case EclStone1Approach: {
            auto& realParams = mlp.template getRealParams<Ewoms::EclStone1Approach>();
...
//            }
        ...
        case EclStone2Approach: {
            auto& realParams = mlp.template getRealParams<Ewoms::EclStone2Approach>();
            realParams.oilWaterParams().drainageParams().setUnscaledPoints(oilWaterUnscaledPointsVector_[satRegionIdx]);
            realParams.oilWaterParams().drainageParams().setEffectiveLawParams(oilWaterEffectiveParamVector_[satRegionIdx]);
            realParams.gasOilParams().drainageParams().setUnscaledPoints(gasOilUnscaledPointsVector_[satRegionIdx]);
            realParams.gasOilParams().drainageParams().setEffectiveLawParams(gasOilEffectiveParamVector_[satRegionIdx]);
...
//            }
        ...
        case EclDefaultApproach: {
            auto& realParams = mlp.template getRealParams<Ewoms::EclDefaultApproach>();
            realParams.oilWaterParams().drainageParams().setUnscaledPoints(oilWaterUnscaledPointsVector_[satRegionIdx]);
            realParams.oilWaterParams().drainageParams().setEffectiveLawParams(oilWaterEffectiveParamVector_[satRegionIdx]);
            realParams.gasOilParams().drainageParams().setUnscaledPoints(gasOilUnscaledPointsVector_[satRegionIdx]);
            realParams.gasOilParams().drainageParams().setEffectiveLawParams(gasOilEffectiveParamVector_[satRegionIdx]);
...
//            }
        ...
        case EclTwoPhaseApproach: {
            auto& realParams = mlp.template getRealParams<Ewoms::EclTwoPhaseApproach>();
            realParams.oilWaterParams().drainageParams().setUnscaledPoints(oilWaterUnscaledPointsVector_[satRegionIdx]);
            realParams.oilWaterParams().drainageParams().setEffectiveLawParams(oilWaterEffectiveParamVector_[satRegionIdx]);
            realParams.gasOilParams().drainageParams().setUnscaledPoints(gasOilUnscaledPointsVector_[satRegionIdx]);
            realParams.gasOilParams().drainageParams().setEffectiveLawParams(gasOilEffectiveParamVector_[satRegionIdx]);
...
//            }
    ...
    EclEpsScalingPoints<Scalar>& oilWaterScaledEpsPointsDrainage(unsigned elemIdx)
    {
        ...
        case EclStone1Approach: {
            auto& realParams = materialParams.template getRealParams<Ewoms::EclStone1Approach>();
            return realParams.oilWaterParams().drainageParams().scaledPoints();
        }

        case EclStone2Approach: {
            auto& realParams = materialParams.template getRealParams<Ewoms::EclStone2Approach>();
            return realParams.oilWaterParams().drainageParams().scaledPoints();
        }

        case EclDefaultApproach: {
            auto& realParams = materialParams.template getRealParams<Ewoms::EclDefaultApproach>();
            return realParams.oilWaterParams().drainageParams().scaledPoints();
        }

        case EclTwoPhaseApproach: {
            auto& realParams = materialParams.template getRealParams<Ewoms::EclTwoPhaseApproach>();
            return realParams.oilWaterParams().drainageParams().scaledPoints();
        }
        default:
            throw std::logic_error("Enum value for material approach unknown!");
    ...
    const Ewoms::EclEpsScalingPointsInfo<Scalar>& oilWaterScaledEpsInfoDrainage(size_t elemIdx) const
...
private:
    ...
    void readGlobalThreePhaseOptions_(const Ewoms::Deck& deck)
    {
        ...
        if (numEnabled == 0) {
            ...
            if (!gasEnabled)
                ...
                twoPhaseApproach_ = Ewoms::EclTwoPhaseGasWater;
            else if (!waterEnabled)
                twoPhaseApproach_ = Ewoms::EclTwoPhaseGasOil;
        }
        else {
            assert(numEnabled == 3);

            threePhaseApproach_ = Ewoms::EclDefaultApproach;
            if (deck.hasKeyword("STONE") || deck.hasKeyword("STONE2"))
                threePhaseApproach_ = Ewoms::EclStone2Approach;
            else if (deck.hasKeyword("STONE1"))
                threePhaseApproach_ = Ewoms::EclStone1Approach;
        }
    }
                                ...
                                const std::vector<int>& imbnumRegionArray)
    {
        ...
        // deck.
        auto gasOilConfig = std::make_shared<Ewoms::EclEpsConfig>();
        auto oilWaterConfig = std::make_shared<Ewoms::EclEpsConfig>();
    ...
    SaturationFunctionFamily getSaturationFunctionFamily(const Ewoms::Deck& deck, const Ewoms::EclipseState& eclState) const
    {
    ...
    void readGasOilEffectiveParametersSgof_(GasOilEffectiveTwoPhaseParams& effParams,
                                            Scalar Swco,
                                            const Ewoms::SgofTable& sgofTable)
    {
    ...
    void readGasOilEffectiveParametersSlgof_(GasOilEffectiveTwoPhaseParams& effParams,
                                             Scalar Swco,
                                             const Ewoms::SlgofTable& slgofTable)
    {
    ...
    void readGasOilEffectiveParametersFamily2_(GasOilEffectiveTwoPhaseParams& effParams,
                                               Scalar /* Swco */,
                                               const Ewoms::Sof3Table& sof3Table,
                                               const Ewoms::SgfnTable& sgfnTable)
    {
    ...
    void readGasOilEffectiveParametersFamily2_(GasOilEffectiveTwoPhaseParams& effParams,
                                               Scalar /* Swco */,
                                               const Ewoms::Sof2Table& sof2Table,
                                               const Ewoms::SgfnTable& sgfnTable)
    {
        // convert the saturations of the SGFN keyword from gas to oil saturations
        std::vector<double> SoSamples(sgfnTable.numRows());
    ...
    void initThreePhaseParams_(const Ewoms::Deck& deck,
                               const Ewoms::EclipseState& /* eclState */,
    ...
    std::vector<Ewoms::EclEpsScalingPointsInfo<Scalar>> unscaledEpsInfo_;
    ...
    Ewoms::EclMultiplexerApproach threePhaseApproach_;
