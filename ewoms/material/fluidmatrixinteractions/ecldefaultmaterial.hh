...
/*!
 * \file
 * \copydoc Ewoms::EclDefaultMaterial
 */
#ifndef EWOMS_ECL_DEFAULT_MATERIAL_HH
#define EWOMS_ECL_DEFAULT_MATERIAL_HH

#include "ecldefaultmaterialparams.hh"
...
public:
    ...
    static Evaluation pcgn(const Params& params,
                           const FluidState& fs)
    {
        const auto& Sw = 1.0 - Ewoms::decay<Evaluation>(fs.saturation(gasPhaseIdx));
    ...
    static Evaluation pcnw(const Params& params,
                           const FluidState& fs)
    {
        const auto& Sw = Ewoms::decay<Evaluation>(fs.saturation(waterPhaseIdx));
    ...
    static Evaluation krg(const Params& params,
                          const FluidState& fluidState)
    {
        const Evaluation& Sw = 1 - Ewoms::decay<Evaluation>(fluidState.saturation(gasPhaseIdx));
    ...
    static Evaluation krw(const Params& params,
                          const FluidState& fluidState)
    {
        const Evaluation& Sw = Ewoms::decay<Evaluation>(fluidState.saturation(waterPhaseIdx));
                          ...
                          const FluidState& fluidState)
    {
        ...
        Evaluation Sw =
            Ewoms::max(Evaluation(Swco),
                         Ewoms::decay<Evaluation>(fluidState.saturation(waterPhaseIdx)));
        Evaluation Sg = Ewoms::decay<Evaluation>(fluidState.saturation(gasPhaseIdx));
        ...
        if (Ewoms::scalarValue(Sw_ow) - Swco < epsilon) {
            Evaluation kro2 = (kro_ow + kro_go)/2;;
            if (Ewoms::scalarValue(Sw_ow) - Swco > epsilon/2) {
    ...
    static void updateHysteresis(Params& params, const FluidState& fluidState)
    {
        ...
        Scalar Sg = Ewoms::scalarValue(fluidState.saturation(gasPhaseIdx));
        ...
        if (params.inconsistentHysteresisUpdate()) {
            ...
            // it like this anyway because (a) the saturation functions of ewoms-material do it
