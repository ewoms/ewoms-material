...
/*!
 * \file
 * \copydoc Ewoms::ThreePhaseParkerVanGenuchten
 */
#ifndef EWOMS_THREE_PHASE_PARKER_VAN_GENUCHTEN_HH
#define EWOMS_THREE_PHASE_PARKER_VAN_GENUCHTEN_HH

#include "threephaseparkervangenuchtenparams.hh"
...
public:
    ...
    static Evaluation pcgn(const Params& params, const FluidState& fluidState)
    {
        ...
        const auto& St =
            Ewoms::decay<Evaluation>(fluidState.saturation(wettingPhaseIdx))
            + Ewoms::decay<Evaluation>(fluidState.saturation(nonWettingPhaseIdx));
        ...
        {
            const Evaluation& x = Ewoms::pow(Se,-1/params.vgM()) - 1;
            return Ewoms::pow(x, 1.0 - params.vgM())/params.vgAlpha();
        }
        ...
        const Evaluation& pc = Ewoms::pow(x, 1.0/params.vgN())/params.vgAlpha();
        const Evaluation& pc_prime =
            Ewoms::pow(x, 1/params.vgN()-1)
    ...
    static Evaluation pcnw(const Params& params, const FluidState& fluidState)
    {
        ...
        if (Se>PC_VG_REG && Se<1-PC_VG_REG) {
            Evaluation x = Ewoms::pow(Se,-1/params.vgM()) - 1.0;
            x = Ewoms::pow(x, 1 - params.vgM());
        ...
        const Evaluation& x = std::pow(Se_regu,-1/params.vgM())-1;
        const Evaluation& pc = Ewoms::pow(x, 1/params.vgN())/params.vgAlpha();
        const Evaluation& pc_prime =
            Ewoms::pow(x,1/params.vgN()-1)
    ...
    static Evaluation krw(const Params& params, const FluidState& fluidState)
    {
        ...
        const Evaluation& r = 1. - Ewoms::pow(1 - Ewoms::pow(Se, 1/params.vgM()), params.vgM());
        return Ewoms::sqrt(Se)*r*r;
    }
    ...
    static Evaluation krn(const Params& params, const FluidState& fluidState)
    {
        ...
        const Evaluation& Sw =
            Ewoms::decay<Evaluation>(fluidState.saturation(wettingPhaseIdx));
        Evaluation Swe = Ewoms::min((Sw - params.Swr()) / (1 - params.Swr()), 1.);
        Evaluation Ste = Ewoms::min((Sw + Sn - params.Swr()) / (1 - params.Swr()), 1.);
        ...
        Evaluation krn_;
        krn_ = Ewoms::pow(1 - Ewoms::pow(Swe, 1/params.vgM()), params.vgM());
        krn_ -= Ewoms::pow(1 - Ewoms::pow(Ste, 1/params.vgM()), params.vgM());
        krn_ *= krn_;
        ...
        if (params.krRegardsSnr())
        {
            ...
            const Evaluation& resIncluded =
                Ewoms::max(Ewoms::min(Sw - params.Snr() / (1-params.Swr()), 1.0), 0.0);
            krn_ *= Ewoms::sqrt(resIncluded );
        }
        else
            krn_ *= Ewoms::sqrt(Sn / (1 - params.Swr()));
    ...
    {
        const Evaluation& Sg =
            Ewoms::decay<Evaluation>(fluidState.saturation(gasPhaseIdx));
        const Evaluation& Se = Ewoms::min(((1-Sg) - params.Sgr()) / (1 - params.Sgr()), 1.);
        ...
        return scaleFactor
            * Ewoms::pow(1 - Se, 1.0/3.)
            * Ewoms::pow(1 - Ewoms::pow(Se, 1/params.vgM()), 2*params.vgM());
    }
};
} // namespace Ewoms
