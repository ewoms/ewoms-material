...
#if !HAVE_ECL_INPUT
#error "The test for the black oil fluid system classes requires ewoms-eclio"
#endif
...
inline void testAll()
{
    ...
    typedef typename Ewoms::MathToolbox<Evaluation>::Scalar Scalar;
    ...
    if (Ewoms::abs(paramCache.maxOilSat() - 0.5) > 1e-10)
        std::abort();

    if (Ewoms::abs(FluidSystem::reservoirTemperature() - (273.15 + 15.555)) > 1e-10)
        std::abort();
    ...
    Ewoms::BlackOilFluidState<Scalar, FluidSystem> fluidState;
    Ewoms::Valgrind::SetUndefined(fluidState);
    ...
    for (unsigned i = 0; i < 1000; ++i) {
        ...
        for (unsigned phaseIdx = 0; phaseIdx < numPhases; ++phaseIdx) {
            ...
            if (Ewoms::abs(FluidSystem::density(fluidState, paramCache, phaseIdx)
            ...
            if (Ewoms::abs(b - bSat) > eps)
                std::abort();

            if (Ewoms::abs(FluidSystem::viscosity(fluidState, paramCache, phaseIdx)
            ...
            if (phaseIdx != waterPhaseIdx && // water is immiscible and thus there is no saturation pressure
                Ewoms::abs(FluidSystem::saturationPressure(fluidState, phaseIdx, regionIdx) - p) > eps*p)
                std::abort();
        }

        if (Ewoms::abs(FluidSystem::bubblePointPressure(fluidState, regionIdx) - p) > eps*p)
            std::abort();

        if (Ewoms::abs(FluidSystem::dewPointPressure(fluidState, regionIdx) - p) > eps*p)
            std::abort();

    }

    // make sure that the {oil,gas,water}Pvt() methods are available
    const auto& gPvt EWOMS_UNUSED = FluidSystem::gasPvt();
    const auto& oPvt EWOMS_UNUSED  = FluidSystem::oilPvt();
    const auto& wPvt EWOMS_UNUSED = FluidSystem::waterPvt();
}
...
int main(int argc, char **argv)
{
    ...
    typedef Ewoms::DenseAd::Evaluation<double, 2> TestEval;
