...
Scalar bringOilToSurface(FluidState& surfaceFluidState, Scalar alpha, const FluidState& reservoirFluidState, bool guessInitial)
{
    ...
    typedef Ewoms::NcpFlash<Scalar, FluidSystem> Flash;
    typedef Ewoms::ThreePhaseMaterialTraits<Scalar, waterPhaseIdx, oilPhaseIdx, gasPhaseIdx> MaterialTraits;
    typedef Ewoms::LinearMaterial<MaterialTraits> MaterialLaw;
    ...
    for (int i = 0;; ++i) {
        if (i >= 20)
            throw Ewoms::NumericalIssue("Newton method did not converge after 20 iterations");
...
inline void testAll()
{
    typedef Ewoms::Spe5FluidSystem<Scalar> FluidSystem;
